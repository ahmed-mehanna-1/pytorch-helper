...
class ModelTrainer(ABC):
    ...
    def __loss_epoch(self, data_dl, device, train=True, progress_bar=True):
        def process(data, running_loss, running_metric):
            ...
            data[target_name] = data[target_name].to(device, non_blocking=True)
            for param in params_name:
                data[param] = data[param].to(device, non_blocking=True)
                params.append(data[param])
            self.optimizer.zero_grad()
            output = self.model(*params)
            params.clear()
            loss_b = self.__loss_batch(output, data[target_name], train)
            with torch.no_grad():
                metric_b = self.__metrics_batch(output, data[target_name])
            running_loss += loss_b
